...
def execute(appResources, appContext):
    ...
    appResources.bibliotecas.file_menagement.create_dirs(raw_folder)


    appResources.bibliotecas.logger.plugin_log(f"raw_file: {raw_file}")
    appResources.bibliotecas.logger.plugin_log(f"https://www.wien.gv.at/ma41datenviewer/downloads/ma41/geodaten/dom_tif/34_4_dom_tif.zip")

    appResources.bibliotecas.logger.update_progress(step_description="Downloading DSM...")
    appResources.bibliotecas.getter.download_file(
    ...
    appResources.bibliotecas.file_menagement.unzip(raw_file, f"{appContext.execution.raw_temp_folder}/dsm/")

    normalized_folder = f"{appContext.execution.normalizer_temp_folder}/dsm"
    normalized_file = f"{normalized_folder}/dsm.tif"
    appResources.bibliotecas.file_menagement.create_dirs(normalized_folder)
    appResources.bibliotecas.logger.update_progress(step_description="Moving files...")
    appResources.bibliotecas.file_menagement.copy(f"{appContext.execution.raw_temp_folder}/dsm/34_4_dom.tif",
                                                  normalized_file)
